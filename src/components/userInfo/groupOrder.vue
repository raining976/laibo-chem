...
<template>
  <div class="groupOrder" :key="reload">
    <div class="top">
      <div class="title">
        {{
          $t("userMenu.groupOrder") +
          "( " +
          $t("cart.total") +
          " : " +
          count +
      ...
      <div class="operateBtn" v-if="funBtn">
        <el-tooltip effect="light" content="通过此订单" placement="top">
          <i
            class="el-icon-check"
            style="font-size: 1.3vw"
            @click="operateProduct(1)"
          ></i>
        </el-tooltip>
        <el-tooltip effect="light" content="拒绝此订单" placement="top">
          <i
            class="el-icon-close"
            style="font-size: 1.3vw"
            @click="operateProduct(2)"
          ></i>
        </el-tooltip>
    ...
    <div class="orderList">
      ...
      <div class="allOrder">
        ...
        >
          <div class="radio">
            <div style="width: 0.94vw; height: 0.94vw">
              <label>
                <input
                  class="checkOne"
                  name="commodity"
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item
                  class="dropdown"
                  :disabled="
                    (item0.status == '待付款' && funBtn == true) ? false : true
                  "
...
export default {
  ...
  data() {
    return {
      ...
      reload: 0, // 更新页面
      //
      funBtn: false, //右上角功能按钮是否显示
      checkBtn: false, //多选单选按钮是否显示
      count: 0,
      checkall: false,
  ...
  async created() {
    //privilege 0-普通成员 1-管理员 2-群主
    if (
      localStorage.getItem("privilege") == 1 ||
      localStorage.getItem("privilege") == 2
    ) {
      this.funBtn = true;
      this.checkBtn = true;
    } else if (localStorage.getItem("privilege") == 0) {
      this.funBtn = false;
      this.checkBtn = false;
    }
  ...
  methods: {
    ...
    async operateProduct(funId) {
      if (this.$data.checkedOrder.length === 0) {
        ...
        // });
        if (funId == 0) {
          await this.delPostReq();
        } else if (funId == 1 || funId == 2) {
          await this.auditOrder(funId);
        }
        this.$data.checkedOrder = [];
        await this.getOrders();
        this.$data.reload++;
    ...
    // 审核订单是否通过（仅允许管理员或创建者使用）
    async auditOrder(isAgree) {
      for (let item of this.$data.checkedOrder) {
        await this.$http
          .post("/auditOrder", {
            agree: isAgree, //1-同意 2-拒绝
            order_no: item.id,
          })
          //回调函数
          .then((res) => {
            if (res.data.code == 20000) {
              this.$message({
                message: this.$t("callback.auditSuccess"),
                type: "success",
              });
              // this.getOrders()
            } else {
...
.operateBtn >>> .el-icon-check:hover {
  color: #04791d;
}
.operateBtn >>> .el-icon-close:hover {
  color: #ff4747;
}
.operateBtn >>> .el-icon-delete:hover {
  color: #ff4747;
}
...
.groupOrder >>> .el-dropdown {
  position: absolute;
  right: 1.25vw;
  bottom: 1.56vw;
}
.groupOrder >>> .el-dropdown-link {
  cursor: pointer;
  font-size: 0.94vw;
  color: #004ea2;
}
...
.groupOrder >>> .el-pagination.is-background .btn-next,
.groupOrder >>> .el-pagination.is-background .btn-prev,
.groupOrder >>> .el-pagination.is-background .el-pager li {
  min-width: 2.08vw;
  border-radius: 0.26vw;
}
.groupOrder >>> .el-icon {
